...
% Given an equivalence relation we can construct the set $A \ \simeq$ which is formed by ``modding out'' the elements in $A$ by $\simeq$.
% For a particular element $a \in A$ we can send it to the set of all the things which it are equivalent to according to $\simeq$
% \[
%   [a] = \{ b \in A \mid a \simeq b \}
% \]
% With this we can define the quotient set of $A$ over $\simeq$.
% \[
%   A \ \simeq = \{ [a] \mid a \in A \}
% \]
